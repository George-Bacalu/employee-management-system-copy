...
class RoleRestControllerMockMvcTest {
    ...
    void getAllRoles_shouldReturnListOfRoles() throws Exception {
        ...
        ResultActions actions = mockMvc.perform(get(API_ROLES).accept(APPLICATION_JSON_VALUE))
    ...
    void getRoleById_withValidId_shouldReturnRolesWithGivenId() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get(API_ROLES + "/{id}", VALID_ID).accept(APPLICATION_JSON_VALUE))
              .andExpect(status().isOk())
    ...
    void getRoleById_withInvalidId_shouldThrowException() throws Exception {
        given(roleService.getRoleById(anyLong())).willThrow(new ResourceNotFoundException(String.format(ROLE_NOT_FOUND, INVALID_ID)));
        mockMvc.perform(get(API_ROLES + "/{id}", INVALID_ID).accept(APPLICATION_JSON_VALUE))
    ...
    void saveRole_shouldAddRoleToList()  throws Exception {
        ...
        MvcResult result = mockMvc.perform(post(API_ROLES).accept(APPLICATION_JSON_VALUE)
    ...
    void updateRoleById_withValidId_shouldUpdateRoleWithGivenId() throws Exception {
        ...
        MvcResult result = mockMvc.perform(put(API_ROLES + "/{id}", VALID_ID).accept(APPLICATION_JSON_VALUE)
                    .contentType(APPLICATION_JSON_VALUE)
    ...
    void updateRoleById_withInvalidId_shouldThrowException() throws Exception {
        given(roleService.updateRoleById(any(RoleDto.class), anyLong())).willThrow(new ResourceNotFoundException(String.format(ROLE_NOT_FOUND, INVALID_ID)));
        mockMvc.perform(put(API_ROLES + "/{id}", INVALID_ID).accept(APPLICATION_JSON_VALUE)
                    .contentType(APPLICATION_JSON_VALUE)
    ...
    void deleteRoleById_withValidId_shouldRemoveRoleWithGivenIdFromList() throws Exception {
        mockMvc.perform(delete(API_ROLES + "/{id}", VALID_ID).accept(APPLICATION_JSON_VALUE))
    ...
    void deleteRoleById_withInvalidId_shouldThrowException() throws Exception {
        doThrow(new ResourceNotFoundException(String.format(ROLE_NOT_FOUND, INVALID_ID))).when(roleService).deleteRoleById(anyLong());
        mockMvc.perform(delete(API_ROLES + "/{id}", INVALID_ID).accept(APPLICATION_JSON_VALUE))
              .andExpect(status().isNotFound())
              .andReturn();
    }
}
